...
class Redmine::I18nTest < ActiveSupport::TestCase
  ...
  def test_get_date_format_string_default
    set_language_if_valid 'en'
    today = Date.today
    Setting.date_format = ''
    assert_equal I18n.backend.translate("en", "date.formats.default"), get_date_format_string
  end

  def test_get_date_format_string
    set_language_if_valid 'en'
    today = Date.today
    Setting.date_format = 'It is %m.%d.%Y'
    assert_equal 'It is %m.%d.%Y', get_date_format_string
  end
